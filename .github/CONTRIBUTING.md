...
- `src/lib/` is the library itself. Each component has one folder containing the framework wrappers (React `*.tsx`, Solid `*.solid.tsx`, Preact `*.preact.tsx`, Vue `*.vue`, Svelte `*.svelte`, Angular `*.angular.ts`, and vanilla `*.ts`) and a shared `*Vanilla.ts` engine where the canvas/WebGL logic lives.
